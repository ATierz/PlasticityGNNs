...
def find_files_nested_folders(folder, files_suffix='.txt'):
    files = []  # List to store found files
    for item in os.listdir(folder):
        item_path = os.path.join(folder, item)
        if os.path.isdir(item_path):
            # If it's a subfolder, dive into it and extend the list with the files found in it
            subfolder_files = find_files_nested_folders(item_path)
            files.extend(subfolder_files)
        elif os.path.isfile(item_path) and item.endswith(files_suffix):
            print(f"Found {files_suffix} file:", item_path)
